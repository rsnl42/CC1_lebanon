...
def overpass_schools(iso2: str, country_name: str, iso3: str) -> gpd.GeoDataFrame | None:
    ...
    """
    # Specific logic for Laos
    if iso3 == "LAO":
        area_query = 'area["name:en"="Laos"][admin_level=2]'
    else:
        area_query = f'area["ISO3166-1"="{iso2}"][admin_level=2]'
        
    # Timeout logic for Ukraine
    timeout = 180 if iso3 == "UKR" else 120
...
out center tags;
"""
    print(f"    Querying Overpass for {country_name} (ISO3: {iso3})...")
    try:
        r = requests.post(
            OVERPASS_URL,
            data={"data": query},
    ...
    if not elements:
        # Fallback for Laos if ISO lookup failed
        if iso3 == "LAO" and area_query == 'area["ISO3166-1"="LA"][admin_level=2]':
             print(f"    Retry Overpass for Laos with name-based lookup...")
             # This part isn't strictly necessary as we set the query above, 
             # but keeping the print for observability.
